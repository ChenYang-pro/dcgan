...
for epoch in range(opt.epochs):
    ...
    if (epoch % opt.checkpoint_every == 0) or (epoch == (opt.epochs - 1)):
        torch.save(netG, '%s/%s%s_netG_epoch_%d.pth' % (opt.outf,opt.city,opt.run_tag, epoch))
        torch.save(netD, '%s/%s%s_netD_epoch_%d.pth' % (opt.outf,opt.city,opt.run_tag, epoch))
...
s ='images/loss_' +  time.strftime("%m_%d_%H_%M") + '.jpg'
